...
class Phase40Application(Phase39Application):
    def __init__(self, *args, **kwargs):
        ...
        self.markets_store = MarketStore(target)
        self.markets = MarketCoordinator(self.markets_store)
        ...
        try:
            if path == "/.well-known/aurora-phase40.json" and method == "GET":
                return self._json_document(
                    ...
                    {
                        "phase": 40,
                        "capability": "evidence-gated global markets and prediction-market surfaces",
                        "layers": list(
                            {
                                "global_stocks",
                                "energy",
            ...
            if path == "/api/public/markets/coverage" and method == "GET":
                return self._response(environ, start_response, 200, self.markets_store.coverage(), rid)

            if path == "/api/public/markets/health" and method == "GET":
                query = self._query(environ)
                maximum_age = int(self._value(query, "max_age_seconds", "0") or 0)
                return self._response(
                    environ, start_response, 200, self.markets_store.health(maximum_age or None), rid
                )

            if path == "/api/public/markets/providers" and method == "GET":
                query = self._query(environ)
                layer = self._value(query, "layer", "")
                rows = self.markets_store.providers(layer)
            ...
            if path == "/api/public/markets/observations" and method == "GET":
                query = self._query(environ)
                rows = self.markets_store.observations(
                    layer=self._value(query, "layer", ""),
                    provider=self._value(query, "provider", ""),
                    limit=int(self._value(query, "limit", "250")),
                )
                return self._response(
                    environ, start_response, 200, {"observations": rows, "total": len(rows)}, rid
                )
            ...
            if path == "/api/public/global-operating-picture" and method == "GET":
                ...
                payload = {
                    ...
                    "markets_configuration": self.markets.configuration(),
                    "markets_providers": self.markets_store.providers(),
                    "recent_markets_runs": self.markets_store.runs(limit=40),
                    "recent_markets_observations": self.markets_store.observations(limit=200),
                    "webcam_coverage": self.webcams.coverage(),
