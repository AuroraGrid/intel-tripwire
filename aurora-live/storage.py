...
class Store:
    def __init__(self,path='aurora-live.db'):
    ...
    def init(self):
        ...
        CREATE TABLE IF NOT EXISTS timeline(id INTEGER PRIMARY KEY AUTOINCREMENT,incident_id TEXT REFERENCES incidents(id) ON DELETE CASCADE,event_type TEXT,summary TEXT,created_at TEXT,payload TEXT);
    ...
    def create_user(self,email,role='analyst'):
        ...
        with self.db() as c:c.execute('INSERT INTO users VALUES(?,?,?,?,?)',(uid,email,hashlib.sha256(token.encode()).hexdigest(),role,created))
        return {'id':uid,'email':email,'role':role,'created_at':created},token
    def auth(self,token):
        if not token:return None
        h=hashlib.sha256(token.encode()).hexdigest()
        with self.db() as c:r=c.execute('SELECT id,email,role,created_at FROM users WHERE token_hash=?',(h,)).fetchone()
    ...
    def add_watchlist(self,user,p):
        ...
        with self.db() as c:c.execute('INSERT INTO watchlists VALUES(?,?,?,?,?,?,?,?)',(wid,user,name,query,dumps(cats),dumps(sevs),conf,created))
        return self.watchlists(user)[0]
    ...
    def ingest(self,payload):
        ...
        for e in payload.get('events',[]):
            iid=str(e.get('id') or sid(e.get('title'),e.get('published_at'))); ids.append(iid)
            with self.db() as c:old=c.execute('SELECT payload,first_seen FROM incidents WHERE id=?',(iid,)).fetchone()
            changed=bool(old and any(loads(old['payload'],{}).get(k)!=e.get(k) for k in ('title','severity','k_align_status','confidence_score','action_state')))
            first=old['first_seen'] if old else now(); seen=now()
            with self.db() as c:
                c.execute('''INSERT INTO incidents VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET title=excluded.title,category=excluded.category,severity=excluded.severity,status=excluded.status,grade=excluded.grade,confidence=excluded.confidence,action=excluded.action,last_seen=excluded.last_seen,payload=excluded.payload''',(iid,e.get('title','Untitled'),e.get('category','world'),e.get('severity','low'),e.get('k_align_status','NOT_PROVEN'),e.get('confidence_grade','G1'),int(e.get('confidence_score',0)),e.get('action_state','MONITOR'),first,seen,dumps(e)))
                for v in e.get('evidence',[]):
                    eid=str(v.get('id') or sid(iid,v.get('url'),v.get('title')))
                    c.execute('''INSERT INTO evidence VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET payload=excluded.payload''',(eid,iid,v.get('source_family','unknown'),v.get('title',''),v.get('url',''),int(bool(v.get('official'))),v.get('published_at'),dumps(v)))
        ...
        alerts=self.match(ids)
        return {'ingested':len(ids),'created':created,'updated':updated,'alerts_created':alerts,'incident_ids':ids}
    ...
    def add_note(self,iid,user,body):
        ...
        with self.db() as c:
            c.execute('INSERT INTO notes VALUES(?,?,?,?,?)',(nid,iid,user,body,created));c.execute('INSERT INTO timeline(incident_id,event_type,summary,created_at,payload) VALUES(?,?,?,?,?)',(iid,'ANALYST_NOTE',body[:240],created,dumps({'note_id':nid})))
