...
def _asset_filter(rows: list[dict[str, Any]], query: dict[str, list[str]]) -> list[dict[str, Any]]:
    kinds = {value.strip() for value in _value(query, "types").split(",") if value.strip()}
    west = float(_value(query, "west", "-180")); east = float(_value(query, "east", "180"))
    south = float(_value(query, "south", "-90")); north = float(_value(query, "north", "90"))
    result=[]
    for row in rows:
...
def dependency_graph(assets: list[dict[str, Any]], incidents: list[dict[str, Any]]) -> dict[str, Any]:
    nodes=[];edges=[];country_nodes={}
    for asset in assets:
        nodes.append({"id":asset["id"],"kind":asset["type"],"label":asset["name"],"latitude":asset.get("latitude"),"longitude":asset.get("longitude")})
        region=str(asset.get("country") or asset.get("region") or "global")
        cid="region:"+region.lower().replace(" ","-")
        if cid not in country_nodes:
            country_nodes[cid]={"id":cid,"kind":"region","label":region};nodes.append(country_nodes[cid])
        edges.append({"source":asset["id"],"target":cid,"type":"located_in","weight":1})
    for incident in incidents[:1000]:
        iid=str(incident.get("id") or "")
        if not iid:continue
        nodes.append({"id":"incident:"+iid,"kind":"incident","label":incident.get("title") or iid,"severity":incident.get("severity")})
        location=str(incident.get("country") or incident.get("location_name") or "global")
        cid="region:"+location.lower().replace(" ","-")
        if cid not in country_nodes:
            country_nodes[cid]={"id":cid,"kind":"region","label":location};nodes.append(country_nodes[cid])
        edges.append({"source":"incident:"+iid,"target":cid,"type":"affects","weight":{"critical":8,"high":4,"medium":2}.get(str(incident.get("severity")),1)})
    return {"nodes":nodes,"edges":edges,"node_count":len(nodes),"edge_count":len(edges)}


def route_exposure(points: list[tuple[float,float]], assets: list[dict[str,Any]], incidents: list[dict[str,Any]], radius: float=5.0) -> dict[str,Any]:
    def near(lat:float,lon:float)->float:
        return min(((lat-a)**2+(lon-b)**2)**0.5 for a,b in points) if points else 999
    exposed_assets=[row for row in assets if near(float(row.get("latitude",0)),float(row.get("longitude",0)))<=radius]
    exposed_incidents=[]
...
class Phase10CompleteApplication(Phase10Application):
    def _all_assets(self, query: dict[str,list[str]]) -> tuple[list[dict[str,Any]],dict[str,int],list[str]]:
        rows=static_assets();counts=defaultdict(int);errors=[]
        ...
        for kind in requested & {"datacenter","pipeline","lng"}:
            try:
                found=overpass_assets(kind);rows.extend(found);counts[kind]+=len(found)
            except Exception as exc:errors.append(kind+": "+str(exc))
        return rows,dict(counts),errors

    def _complete(self,path:str,query:dict[str,list[str]])->dict[str,Any]:
        started=time.perf_counter();incidents=self._incidents(query)
        if path=="/api/platform/geo/countries":
            return {"geojson":country_features(),"dossiers":world_bank_countries()}
        ...
        if path=="/api/platform/geo/assets":data={"assets":_asset_filter(assets,query),"counts":counts,"errors":errors}
        elif path=="/api/platform/geo/catalog":data={**catalog_summary(counts),"errors":errors}
            ...
            raw=_value(query,"points");points=[]
            for pair in raw.split(";"):
                if not pair.strip():continue
                lat,lon=pair.split(",",1);points.append((float(lat),float(lon)))
    ...
    def __call__(self,environ,start_response):
        ...
        try:
            ...
            self._user(environ);query=urllib.parse.parse_qs(str(environ.get("QUERY_STRING") or ""),keep_blank_values=True)
            return self._response(environ,start_response,200,self._complete(path,query),rid)
