...
class WorkerTests(unittest.TestCase):
    def setUp(self):
        ...
        if self.store.backend == "postgres":
            with self.store.db() as connection:
                for table in ["worker_heartbeats", "worker_jobs", "deliveries", "webhooks", "notes", "alerts", "timeline", "evidence", "incidents", "watchlists", "users"]:
                    connection.execute(f"DELETE FROM {table}")
        self.user, _ = self.store.create_user("worker@example.com", "admin")
    ...
    def test_refresh_queues_new_alert_deliveries(self):
        self.store.add_watchlist(self.user["id"], {"name": "Ports", "query": "port"})
        self.ops.add_webhook(self.user["id"], {"name": "Ops", "url": "https://hooks.example.com/aurora"})
    ...
    def test_retry_backoff_and_dead_letter(self):
        ...
        alert = self.store.alerts(self.user["id"])[0]
        self.ops.queue_deliveries(self.user["id"], alert["id"])
        ...
        with self.store.db() as connection:
            row = connection.execute("SELECT status,next_attempt_at FROM deliveries").fetchone()
            self.assertEqual(row["status"], "failed")
            self.assertIsNotNone(row["next_attempt_at"])
            connection.execute("UPDATE deliveries SET next_attempt_at=?", (now(),))
        second = queue.deliver_due(opener=failing)
        self.assertEqual(second["dead"], 1)
        with self.store.db() as connection:
            row = connection.execute("SELECT status,dead_lettered_at FROM deliveries").fetchone()
